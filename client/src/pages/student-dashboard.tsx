...
import { Calendar, Clock, GraduationCap, DollarSign, Car } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Progress } from "@/components/ui/progress";
import type { Student, Lesson, Payment } from "@shared/schema";
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Track your progress and manage your driving lessons</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-hours-completed">
              {hoursCompleted}
            </div>
            <p className="text-xs text-muted-foreground">of {totalHours} required hours</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-upcoming-lessons">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-outstanding-amount">
              ${recentPayments.filter(p => p.status === "pending").reduce((sum, p) => sum + parseFloat(p.amount), 0).toFixed(2)}
            </div>
      ...
      <Card className="mb-8">
        ...
        <CardContent>
          <Progress value={progressPercentage} className="mb-2" data-testid="progress-bar-learning" />
          <p className="text-sm text-muted-foreground">
            {hoursCompleted} of {totalHours} hours completed • {(totalHours - hoursCompleted).toFixed(1)} hours remaining
          </p>
        </CardContent>
      </Card>
          ...
          <CardContent>
            {upcomingLessons.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground mb-4">No upcoming lessons scheduled</p>
                <Button variant="outline" size="sm">Schedule Your First Lesson</Button>
              </div>
            ) : (
                ...
                {upcomingLessons.map((lesson) => (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground capitalize mt-1">{lesson.lessonType}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {recentPayments.map((payment) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{payment.description || "Lesson payment"}</p>
                    </div>
                    <Badge
                      variant={
