...
export default function InstructorDashboard() {
  ...
  const completedToday = todaysLessons.filter(l => l.status === "completed").length;
  const totalEarningsToday = todaysLessons.filter(l => l.status === "completed").length * 50; // Estimate

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Instructor Dashboard</h1>
        <p className="text-muted-foreground">Manage your students and schedule</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-todays-lessons">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-todays-earnings">
              ${totalEarningsToday}
            </div>
            <p className="text-xs text-muted-foreground">from {completedToday} lessons</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <CardContent>
            {todaysLessons.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground mb-4">No lessons scheduled for today</p>
                <Button variant="outline" size="sm">Schedule a Lesson</Button>
              </div>
            ) : (
                ...
                {todaysLessons.map((lesson) => (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      {lesson.location && (
                        <p className="text-xs text-muted-foreground mt-1">{lesson.location}</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-2">
            ...
            <Button size="sm" variant="outline" data-testid="button-view-all-students">
              View All
            </Button>
          </CardHeader>
          <CardContent>
            {students.length === 0 ? (
              ...
              <div className="space-y-4">
                {students.slice(0, 5).map((student) => {
                  const initials = `${student.user?.firstName?.[0] || ""}${student.user?.lastName?.[0] || ""}`.toUpperCase() || "S";
                  ...
                  return (
                    ...
                    >
                      <Avatar>
                        <AvatarImage src={student.user?.profileImageUrl || undefined} className="object-cover" />
